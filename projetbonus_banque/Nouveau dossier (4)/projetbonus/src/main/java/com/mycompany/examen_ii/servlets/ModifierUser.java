...
public class ModifierUser extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        //processRequest(request, response);
/*
        String user = request.getParameter("data_user_modification");
        UserDAO userDAO = new UserDAO();
        List<UserDTO> listuserDTO = null;
        HttpSession session = request.getSession();

        try {
            listuserDTO=userDAO.ModificationUser(Integer.parseInt(user));
            this.getServletContext().getRequestDispatcher("/Views/liste.jsp").forward(request, response);
